...
```bash
# 安裝 Husky
pnpm add -D husky # 或 npm/yarn

# 初始化 Husky (創建 .husky 目錄)
pnpm exec husky init # 或 npx/yarn dlx
```

在每次 commit 時對 *整個* 代碼庫運行 linter 會非常緩慢。`lint-staged` 確保你只檢查當前正在提交的文件。

```bash
pnpm add -D lint-staged # 或 npm/yarn
```
...
```bash
pnpm exec lint-staged # 或 npx/yarn dlx
```
...
# npm
pnpm add -D @evilmartians/lefthook # 或 npm/yarn
...
```bash
pnpm add -D @commitlint/cli @commitlint/config-conventional # 或 npm/yarn

# 創建配置
echo "export default { extends: ['@commitlint/config-conventional'] };" > commitlint.config.js

# 添加 hook
echo "pnpm exec commitlint --edit \$1" > .husky/commit-msg # 或 npx/yarn dlx
```
...
```bash
pnpm add -D cz-conventional-changelog # 或 npm/yarn
```
...
使用 `pnpm run commit`（或 `pnpm exec cz`）替代 `git commit`，系統將交互式地引導你填寫 type、scope 和 description。
...
| **環境不匹配** | 在 hook 腳本中使用絕對路徑或標準命令（如 `pnpm run`）。 |
