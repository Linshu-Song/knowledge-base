...
- [English](./operations-troubleshooting) - 英文
- [简体中文](./operations-troubleshooting) - 簡體中文
- **繁體中文** (當前) - 本文檔
...
*上一部分：[第四部分：生產環境部署](./production-deployment)*
...
| 部分 | 主題 | 關鍵要點 |
|------|-------|-------------|
| [第一部分](./architecture-overview) | 架構概覽 | 根據團隊規模和發布節奏選擇單一儲存庫與多儲存庫 |
| [第二部分](./infrastructure-setup) | 基礎架構設定 | 分層 Compose 檔案 + 多階段 Dockerfile |
| [第三部分](./development-environment) | 開發環境 | 開發容器 + SSH Agent 轉送 |
| [第四部分](./production-deployment) | 生產環境部署 | 預備環境優先晉升、密鑰管理器、路徑過濾 CI/CD |
| [第五部分](./operations-troubleshooting) | 操作與故障排除 | 健康檢查、回滾程序、常見修復 |
