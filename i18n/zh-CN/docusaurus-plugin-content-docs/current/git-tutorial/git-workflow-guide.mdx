...
- **暂存区 vs 仓库**：暂存区保存*下一次*提交的更改（临时），而仓库存储*整个*提交历史（永久）。只有 `git commit` 才能将更改从暂存区移动到仓库。
...
:::danger[关键规则]
1. **切勿强制推送到共享分支**（`main`、`develop`）- 改用 `--force-with-lease`
2. **切勿改写已发布的历史** - 已推送的提交应视为不可变
3. **执行破坏性操作前先检查** - 运行 `git status` 和 `git diff`
...
管理**多个工作目录**，它们都指向同一个仓库，让你可以同时检出不同分支而无需 stash 或切换。
...
:::caution
同一分支**同时只能在一个工作树中**检出。尝试在两个工作树中检出相同分支会失败。
:::
...
*   [📄 子模块 (Submodules) 指南](./git-composition-strategy/submodule.mdx) — 管理外部依赖。
