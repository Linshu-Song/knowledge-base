...
```bash
# Install Husky
pnpm add -D husky # or npm/yarn

# Initialize Husky (creates .husky directory)
pnpm exec husky init # or npx/yarn dlx
```
...
```bash
pnpm add -D lint-staged # or npm/yarn
```
...
```bash
pnpm exec lint-staged # or npx/yarn dlx
```
...
# npm
pnpm add -D @evilmartians/lefthook # or npm/yarn
...
```bash
pnpm add -D @commitlint/cli @commitlint/config-conventional # or npm/yarn

# Create config
echo "export default { extends: ['@commitlint/config-conventional'] };" > commitlint.config.js

# Add hook
echo "pnpm exec commitlint --edit \$1" > .husky/commit-msg # or npx/yarn dlx
```
...
```bash
pnpm add -D cz-conventional-changelog # or npm/yarn
```
...
Use `pnpm run commit` (or `pnpm exec cz`) instead of `git commit` to be prompted for type, scope, and description.
...
| **Environment mismatch** | Use absolute paths or standard commands (e.g., `pnpm run`) in hook scripts. |
